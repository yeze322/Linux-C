...
void shell_command(int fd_client)
{
    char fileName[30]="";
    while(recv(fd_client,fileName,30,0)==0)
    memset(fileName,0,30);
    printf("shell command  arrived: %s\n",fileName);
    char *command[] = {"zero",fileName,NULL};
    if(fork()==0){
        printf("child start\n");
        per(execv("./_shell/shell.sh",command));
        
        printf("if print here, failed !\n");
        exit(1);
    }else{
        wait(NULL);
    }
    printf("finish shell command\n");
}
...
void son(int fd_client)
{
    while(1){
        ...
        if(strcmp(dowhat,"shell")==0||strcmp(dowhat,"sh")==0){
            printf("\n\n====start shell====\n\n");
            shell_command(fd_client);
            printf("\n\n====end shell====\n\n");
            continue;
        }
