...
private:
	...
	void preorderBST(const BST&);
	Node* preorderBST(Node*);
	/* 可选函数 */
...
/* preorder */
void BST::preorderBST(const BST& origin)
{
	if(origin.head == NULL)
		return;
	head = preorderBST(origin.head);
}
Node* BST::preorderBST(Node *src)
{
	if(src == NULL)
		return NULL;
	Node *p = new Node(src->val);
	p->left = preorderBST(src->left);
	p->right = preorderBST(src->right);
	return p;
}
/* ----- */
/* insert new node */
