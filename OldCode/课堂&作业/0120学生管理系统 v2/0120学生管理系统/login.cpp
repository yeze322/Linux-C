...
addag:
	...
	dayinzhanghuliebiao();
	printf("\n\n\n		添加用户，按提示填写\n		=======================\n\n");
	cout<<"		输入要添加的用户名[输入-1取消本次操作]:";
	fflush(stdin);
	char name[NAMELEN]="";cin>>name;
	...
	cout<<"\n		将要添加的用户名为 "<<name;
	cout<<" 输入密码：";char password[PSWDLEN]="";fflush(stdin);cin>>password;
...
delag:
	...
	while(visit->next){
		...
		{
			if(visit->permission==1)
			{
				printf("\n		管理原账户不允许删除！请先更改权限再操作！");
				system("pause");
				goto delag;
			}
			dayinUSR(visit->next);
			printf("\n		确认要删除该用户吗？y确认，其他键返回");
			char yi;fflush(stdin);cin>>yi;fflush(stdin);
			if(yi!='y')
				goto delag;
...
void RewirteLOGIN()
{
	FILE *fp=fopen(UsrSrc,"w");
	USR *visit=U->head->next;
	while(visit)
	{
		fprintf(fp,"%d %s %s\r\n",visit->permission,visit->userid,visit->password);
		visit=visit->next;
	}
	fclose(fp);
}
